...
int ems_setup(char const* req_pipe_path, char const* resp_pipe_path, char const* server_pipe_path) {
    ...
    if (mkfifo(req_pipe_path, 0604) < 0) return 1;      // Create request pipe
    if (mkfifo(resp_pipe_path, 0604) < 0) return 1;     // Create response pipe
    ...
    if (read(resp_pipe_fd, read_session_id, sizeof(int)) < 0) return 1;
